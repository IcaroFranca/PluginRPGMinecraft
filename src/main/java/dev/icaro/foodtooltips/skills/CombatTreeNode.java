...
public record CombatTreeNode(CombatAbility ability, CombatBranch branch, List<CombatAbility> prerequisites,
    ...
    static {
        ...
        // (unconditional crit-damage multiplier, the branch's real finisher). Max ranks
        // climb with tier too (10/12/12/15/20) — deeper nodes are a longer grind, not
        // just a pricier one.
        register(CombatAbility.RUTHLESS_STRIKES, CombatBranch.FURY, 10, Kind.PASSIVE, 51);
        register(CombatAbility.EXECUTIONER, CombatBranch.FURY, 12, Kind.PASSIVE, 42, CombatAbility.RUTHLESS_STRIKES);
        register(CombatAbility.ARMOR_PIERCER, CombatBranch.FURY, 12, Kind.PASSIVE, 33, CombatAbility.EXECUTIONER);
        register(CombatAbility.BERSERKER, CombatBranch.FURY, 15, Kind.PASSIVE, 24, CombatAbility.ARMOR_PIERCER);
        register(CombatAbility.CRITICAL_MASTERY, CombatBranch.FURY, 20, Kind.PASSIVE, 15, CombatAbility.BERSERKER);

        // ---- Sustain (blood) ----
        register(CombatAbility.VAMPIRISM, CombatBranch.SUSTAIN, 10, Kind.PASSIVE, 49);
        register(CombatAbility.BLOOD_LUST, CombatBranch.SUSTAIN, 12, Kind.PASSIVE, 40, CombatAbility.VAMPIRISM);
        register(CombatAbility.TREASURE_HUNTER, CombatBranch.SUSTAIN, 10, Kind.PASSIVE, 41, CombatAbility.VAMPIRISM);
        register(CombatAbility.UNDYING_WILL, CombatBranch.SUSTAIN, 15, Kind.PASSIVE, 31, CombatAbility.BLOOD_LUST);
        register(CombatAbility.SOUL_HARVEST, CombatBranch.SUSTAIN, 15, Kind.PASSIVE, 22, CombatAbility.UNDYING_WILL);
        register(CombatAbility.SECOND_WIND, CombatBranch.SUSTAIN, 20, Kind.PASSIVE, 13, CombatAbility.SOUL_HARVEST);
        ...
        register(CombatAbility.SWORD_THROW, CombatBranch.UTILITY, 15, Kind.ACTIVE_KEYBIND, 38, CombatAbility.HUNTERS_INSTINCT);
        register(CombatAbility.CLEAVE, CombatBranch.UTILITY, 15, Kind.PASSIVE, 29, CombatAbility.SWORD_THROW);
        register(CombatAbility.RELENTLESS, CombatBranch.UTILITY, 20, Kind.PASSIVE, 20, CombatAbility.CLEAVE);

        // ---- Synergy (bridges between branches) ----
        register(CombatAbility.VITAL_TOUCH, CombatBranch.SYNERGY, 15, Kind.ACTIVE_MENU, 39, CombatAbility.VAMPIRISM);
        register(CombatAbility.COMBAT_MASTERY, CombatBranch.SYNERGY, 20, Kind.PASSIVE, 30,
                CombatAbility.BLOOD_LUST, CombatAbility.SWORD_THROW);
        register(CombatAbility.ARCANE_SLASH, CombatBranch.SYNERGY, 15, Kind.ACTIVE_MENU, 34, CombatAbility.EXECUTIONER);

        // ---- Capstone ---- the longest grind in the tree: 25 ranks.
        register(CombatAbility.APEX_WARRIOR, CombatBranch.CAPSTONE, 25, Kind.PASSIVE, 4,
